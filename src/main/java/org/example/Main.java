...
public class Main {
  private static final UtilsMethod utilsMethod = new UtilsMethod();
  ...
  public static void useCIN(Scanner scanner) {
    utilsMethod.useUniqueItem(
        scanner,
        actualWallet.getCin(),
        scanner1 -> useCIN(scanner1),
        s -> actualWallet.setCin(new CIN(scanner)));
  }
    ...
    switch (choice) {
      ...
      case 2:
        ...
        for (CreditCard creditCard : actualWallet.getCreditCards()) {
          messages.add(
              "" + actualWallet.getCreditCards().indexOf(creditCard) + ". " + creditCard.getName());
        }
        int choice2 = utilsMethod.askUser(messages);
        if (actualWallet.getCreditCards().size() > choice2) {
          utilsMethod.useUniqueItem(
              scanner,
              actualWallet.getCreditCards().get(choice2),
              scanner1 -> useCreditCard(scanner1),
              null);
        } else {
          System.out.println("Choix invalide. Veuillez réessayer.");
          useCreditCard(scanner);
  ...
  public static void useDrivingLicense(Scanner scanner) {
    utilsMethod.useUniqueItem(
        scanner,
        actualWallet.getDrivingLicense(),
        scanner1 -> useDrivingLicense(scanner1),
        s -> actualWallet.setDrivingLicense(new DrivingLicense(scanner)));
  }
  ...
  public static void useIdPhoto(Scanner scanner) {
    utilsMethod.useUniqueItem(
        scanner,
        actualWallet.getIdPhoto(),
        scanner1 -> useIdPhoto(scanner1),
        s -> actualWallet.setIdPhoto(new IdPhoto("ma photo")));
  }
  ...
  public static void useMoney(Scanner scanner) {
    utilsMethod.useMultiItems(
        scanner,
        actualWallet.getMoney(),
        scanner1 -> useMoney(scanner1),
        s -> actualWallet.setMoney(new Money(s)),
        integer -> {
  ...
  public static void useVisitCard(Scanner scanner) {
    utilsMethod.useMultiItems(
        scanner,
        actualWallet.getVisitCard(),
        scanner1 -> useVisitCard(scanner1),
        s -> actualWallet.setVisitCard(new VisitCard(scanner)),
        integer -> {
